locals {
  container_ip         = "${var.runner.host.ip_addr}${var.runner.host.cidr}"
...
module "lxc" {
  ...
  hostname       = var.runner.host.name
  description    = var.proxmox.host_description
  template_vmid  = var.runner.host.template_vmid
  node_name      = var.runner.host.node_name
  vmid           = try(var.runner.host.vmid, null)
  cores          = var.runner.host.cores
  memory         = var.runner.host.memory
  disk_size      = var.runner.host.disk_size
  datastore_id   = var.runner.host.datastore_id
  network_bridge = var.runner.host.network_bridge
  vlan_id        = nonsensitive(var.runner.host.vlan_id)
  ip_address     = local.container_ip
  gateway        = var.runner.host.gateway
...
resource "ansible_playbook" "deploy" {
  ...
  extra_vars = {
    ansible_host                 = var.runner.host.ip_addr
    ansible_user                 = "root"
